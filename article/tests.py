...
class ArticleTestCase(TestCase):
    ...
    def test_user_added_slug_unique(self):
            # 1. Create a baseline article with a specific slug
            start_slug = "my-custom-slug"
            Article.objects.create(title="First Article", slug=start_slug)
            
            # 2. Try to create a second article using the EXACT SAME slug manually
            title = "Second Article"
            # We explicitly pass the slug here to simulate a user typing it
            obj = Article.objects.create(title=title, slug=start_slug)
            
            # 3. Assert that the slug was changed to avoid a crash
            # The new slug should NOT be the same as the starting one
            self.assertNotEqual(obj.slug, start_slug)
            
            # The new slug should still look similar (e.g., "my-custom-slug-2")
